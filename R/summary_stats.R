...
uni_ts_summary <- function(obs, times = NULL, effort = NULL, 
  ...
  if (!is.null(dim(obs)) | !is.numeric(obs)){
    stop("`obs` must be a numeric single-dimension vector")
  }
  if (is.null(times)){
    message("`time` is `NULL`, assuming evenly spaced data")
    times <- 1:nrow(obs)
...
#'   observations.
#'
#' @param method \code{character} representing a function name used to 
#'   interpolate \code{obs}. Defaults to \code{"\link[forecast]{na.interp}"}.
#'
